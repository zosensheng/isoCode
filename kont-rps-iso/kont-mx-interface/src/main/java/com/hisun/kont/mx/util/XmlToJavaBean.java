...
import com.hisun.kont.mx.bizdata.BizData;
...
public class XmlToJavaBean {
    ...
    public Document getDocument(String xml) {
        ...
        try(ByteArrayInputStream bais = new ByteArrayInputStream(xml.getBytes("UTF-8"))) {
            ...
            KontException.throwKontException("XML_TO_JAVABEAN_FAIL", ex);
        }
        return document;
    }
    ...
    public Object getAppHdrByBizDataElement(Element bizDataElement) {
        ...
        try {
            ...
            appHdrStr = "<AppHdr xmlns=\"urn:iso:std:iso:20022:tech:xsd:head.001.001.02\">" + appHdrStr.substring(appFirEnId + 1);

            appHdr = toBean(appHdrStr, BusinessApplicationHeaderV02.class);
        } catch (Exception ex) {
    ...
    public BizData generteJavaBean(String xml) {
        ...
        BusinessApplicationHeaderV02 appHdr = null;
        Object doc = null;
        for (Element bizdata : bizDataElementList) {
            appHdr = (BusinessApplicationHeaderV02) getAppHdrByBizDataElement(bizdata);
            try {
                ...
                KontException.throwKontException("XML_TO_JAVABEAN_FAIL", e);
            }
        }
        // 如果解析后信息还是不存在，则报错
        if (JudgeUtils.isNull(appHdr) || JudgeUtils.isNull(doc)) {
            ...
            KontException.throwKontException("XML_TO_JAVABEAN_FAIL");
        }
